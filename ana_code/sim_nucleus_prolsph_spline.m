...
function [cutca,h,sol,rfun,zfun,tfun]=sim_nucleus_prolsph_spline(D,l,a,cutangle,crtpts,fits,tint,tmax,plotBool)
interval=0.01;
L=sqrt((l/2)^2+a^2);
r=0:interval:a;
zpos=0:interval:l;
...
function [xifun,zfun,just_tfun,sol]=generatecaconc(a,l,D,r,t,crtpts,fits)
if isempty(D)
    D=[];
end
if size(fits,1)>1
...
sol=cell(size(t));
xi=0:0.01:(L/a);
c_mk=[0.122966,0.143927,0.166263,0.189769,0.214258,0.239571]*1e2;
b_m=[0.0115295,0.0679778,0.13789,0.222337,0.322179,0.438099]*1e3;
A_mk=[6.02629,5.20633,4.50939,3.92207,3.42978,3.01847];
d=zeros(6,maxt);
for m=1:6
    for n=1:maxt
        d(m,n)=0;
...
xifun=besselj(0,r'*alphan(2:end))./besselj(1,alphan(2:end)*a)./(alphan(2:end));
zfun=sin(z'*(2*(0:(maxt-1))+1)*pi/l)./(2*(0:(maxt-1))+1);
sol=cellfun(@(x)zeros(size(xifun(:,1)*zfun(:,1)')),sol,'Un',0);
common_coeff=8/(pi*a);
...
for time=1:size(t,2)
    ...
    just_tfun{time}=-common_coeff*(intofe-0.01*exp(-D*(alphan(n+1)'.^2+((2*m+1)*pi/l).^2)*t(time)));
    sol{time}=xifun*just_tfun{time}*zfun';
    ...
    if fitNum==0
        qadd=fits{end}(crtpts(end));
    elseif fitNum >1 &fitNum<3
        qadd=ppval(fits{1},(t(time)));
    elseif fitNum<=size(fits{1}.coefs,1)
        qadd=ppval(fits{1},(t(time)));
    else
        qadd=fits{2}(t(time));
    end
...
function sol=sphbesselj(n,x)
xis0=x==0;
xn0=x>0;
solis0=xis0;
soln0=sqrt(pi./2*xn0).*besselj(n+0.5,xn0);
sol=[solis0,soln0];
end