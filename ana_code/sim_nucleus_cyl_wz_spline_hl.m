...
function [cutca,h,sol,rfun,zfun,tfun]=sim_nucleus_cyl_wz_spline_hl(D,l,a,cutangle,heightInc,lengthInc,crtpts,fits,tint,tmax,plotBool)
interval=0.01;
r=0:interval:a;
zpos=0:interval:l;
% maxn=100;
...
[rfun,zfun,tfun,sol]=generatecaconc(a,l,D,r,t,crtpts,fits,heightInc,lengthInc);
...
function [rfun,zfun,just_tfun,sol]=generatecaconc(a,l,D,r,t,crtpts,fits,incheight,inclength)
if isempty(D)
    D=[];
end
if size(fits,1)>1
...
for time=1:size(t,2)
    n=1:maxt;
    m=0:(maxt-1);
    intofe=int_e_fit(D,m,n,alphan,l,crtpts,fits,t(time),incheight,inclength);
    ...
    elseif fitNum >1 &fitNum<7
        qadd=incheight*(ppval(fits{1},(t(time)))-ppval(fits{1},fits{1}.breaks(1)))+ppval(fits{1},fits{1}.breaks(1));
    elseif fitNum<=size(fits{1}.coefs,1)+1
        qadd=incheight*(ppval(fits{1},(t(time)*inclength))-ppval(fits{1},fits{1}.breaks(1)))+ppval(fits{1},fits{1}.breaks(1));
    end
    NE_release=0;%.1;%2*(qadd>0.1);
    sol{time}=sol{time}+qadd+NE_release;
    if sum(abs(sol{time}(:))>3*(qadd+NE_release))>0
...
function sol=int_e_fit(D,m,n,alphan,l,crtpts,fits,time,incheight,inclength)
...
for ii=2:min((numSegs-1),7)
    ...
    segInts(:,:,ii)=segInts(:,:,ii)*incheight;
	catch
        warning('weird fit?')
    end
end
if numSegs>7
    newtime=ptime+(time-ptime)*inclength;
    newcrtpts=ptime+(crtpts-ptime)*inclength;
for ii=8:(numSegs-1)
    try
    segInts(:,:,ii)=(exp(ecoeff*(newcrtpts(ii)-newtime)).*...
        (3*fits{1}.coefs(ii-1,1)*(ecoeff.^2*newcrtpts(ii)^2-2*ecoeff*newcrtpts(ii)+2)...
        +2*(fits{1}.coefs(ii-1,2)-3*fits{1}.coefs(ii-1,1)*fits{1}.breaks(ii-1))*ecoeff.*(ecoeff*newcrtpts(ii)-1)...
        +ecoeff.^2*(fits{1}.coefs(ii-1,3)+3*fits{1}.coefs(ii-1,1)*fits{1}.breaks(ii-1).^2-2*fits{1}.coefs(ii-1,2)*fits{1}.breaks(ii-1)))...
        -exp(ecoeff*(newcrtpts(ii-1)-newtime)).*...
        (3*fits{1}.coefs(ii-1,1)*(ecoeff.^2*newcrtpts(ii-1)^2-2*ecoeff*newcrtpts(ii-1)+2)...
        +2*(fits{1}.coefs(ii-1,2)-3*fits{1}.coefs(ii-1,1)*fits{1}.breaks(ii-1))*ecoeff.*(ecoeff*newcrtpts(ii-1)-1)...
        +ecoeff.^2*(fits{1}.coefs(ii-1,3)+3*fits{1}.coefs(ii-1,1)*fits{1}.breaks(ii-1).^2-2*fits{1}.coefs(ii-1,2)*fits{1}.breaks(ii-1))))./ecoeff.^3;
    segInts(:,:,ii)=segInts(:,:,ii)*incheight;
	catch
        warning('weird fit?')
...
ii=numSegs;
segInts(:,:,ii)=((3*fits{1}.coefs(ii-1,1)*(ecoeff.^2*newtime^2-2*ecoeff*newtime+2)...
        +2*(fits{1}.coefs(ii-1,2)-3*fits{1}.coefs(ii-1,1)*fits{1}.breaks(ii-1))*ecoeff.*(ecoeff*newtime-1)...
        +ecoeff.^2*(fits{1}.coefs(ii-1,3)+3*fits{1}.coefs(ii-1,1)*fits{1}.breaks(ii-1).^2-2*fits{1}.coefs(ii-1,2)*fits{1}.breaks(ii-1)))...
        -exp(ecoeff*(newcrtpts(ii-1)-newtime)).*...
        (3*fits{1}.coefs(ii-1,1)*(ecoeff.^2*newcrtpts(ii-1)^2-2*ecoeff*newcrtpts(ii-1)+2)...
        +2*(fits{1}.coefs(ii-1,2)-3*fits{1}.coefs(ii-1,1)*fits{1}.breaks(ii-1))*ecoeff.*(ecoeff*newcrtpts(ii-1)-1)...
