...
function [sol,rfun,zfun,tfun]=sim_nucleus_cyl_wz_spline_no_cp(D,l,a,fits,tint,tmax)
interval=0.01;
r=0:interval:a;
% zpos=0:interval:l;
% maxn=100;
...
[rfun,zfun,tfun,sol]=generatecaconc(a,l,D,r,t,fits);
end
function [rfun,zfun,just_tfun,sol]=generatecaconc(a,l,D,r,t,fits)
if isempty(D)
    D=[];
end
if size(fits,1)>1
...
for time=1:size(t,2)
    n=1:maxt;
    m=0:(maxt-1);
    intofe=int_e_fit(D,m,n,alphan,l,fits,t(time));
    ...
    fitNum=find(fits{1}.breaks>t(time),1,'first')-1;
    if fitNum==0
        qadd=ppval(fits{1},fits{1}.breaks(1));
    elseif fitNum<=size(fits{1}.breaks,2)
        qadd=ppval(fits{1},(t(time)));
    else
        qadd=ppval(fits{1},fits{1}.breaks(end));
    end
...
ii=numSegs;
if numSegs==(size(crtpts,2)+1)
    segInts(:,:,ii)=zeros(size(n,2),size(m,2));
else
    try
