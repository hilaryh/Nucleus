function sol=fitDinNuc(dataStruct,stageit,hlen,llen,a,l,typeit,cellit,redoBool,maxcvals)
if (1-redoBool) && ~isempty(dataStruct(stageit).Dvals)
    sol=dataStruct(stageit).Dvals;
    return
end
cutangle=dataStruct(stageit).cutangle;
nucSlice=dataStruct(stageit).nucSlice;
...
%         x(peakit)=fminsearch(leastsquarescomp,12873); %33053 %19261 %15544
        ...
        ncslice=num2cell(nucSlice(:,peakstart:peakend)',2)';
        size(ncslice{1});
        size(simslice2{1});
...
function sol=callcylfun(D,l,a,cutangle,qvars,varchoice,tint,tmax,nucSlicex)
    if D>0
        [cutca,h]=sim_nucleus_cyl_wz(D,l,a,cutangle,qvars,tint,tmax);
        if varchoice==0
            cutcax=[-h(end:-1:2) h];
