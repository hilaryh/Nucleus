...
if param_type
    ...
    b_t = 6e3*reshape(repmat(0:(1/ns):1,[numEachVar^5,1]),numEachVar^6,1);
    kn  = 1+1e4*repmat(reshape(repmat((0:(1/ns):1),[numEachVar^3,1]),numEachVar^4,1),[numEachVar^2,1]);
    kp  = 1+1.4*repmat(reshape(repmat((0:(1/ns):1),[numEachVar^2,1]),numEachVar^3,1),[numEachVar^3,1]);
    NPC = 0.5+1.5*repmat(reshape(repmat((0:(1/ns):1),[numEachVar,1]),numEachVar^2,1),[numEachVar^4,1]);
    a   = 2*ones(numSteps,1); 
...
params=[D_b,b_t,kp,kn,NPC,a];
for pos = 1%1:4
fits=dataStruct(cellit).fstfit{pos};
crtpts=dataStruct(cellit).crtpts{pos};
tint=10;
tmax=3000;
results=zeros(numSteps,11);
parfor ii=1:numSteps
    [sol,r,t]       =nonlinearDiff_notfast(D_u(ii),params(ii,:),fits,crtpts,tint,tmax);
...
save(strcat(savedir,'parPS_nf',num2str(pos)),'results')
end
