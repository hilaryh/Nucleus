...
%           double check bd
function [sol,r,t]=nonlinearDiff_hl(D_uin,params_in,heightInc,lengthInc,data_fits,data_crtpts,tint,tmax)
...
global fits; global crtpts;global params; global D_u; global incheight; global inclength;
fits=data_fits;
crtpts=data_crtpts;
incheight=heightInc; inclength=lengthInc;
...
global fits; global crtpts; global params;global incheight; global inclength;
pl=0*ones(size(t));
ql=0*ones(size(t));
qr=0*ones(size(t));
...
tpostpeak=t>=ptime;
pr=t;
pr(tprepeak)=ur-params(4)*(incheight*(q_spline(t(tprepeak),fits,crtpts)-q_spline(0,fits,crtpts))+q_spline(0,fits,crtpts));
pr(tpostpeak)=ur-params(4)*(incheight*(q_spline(ptime+(t(tpostpeak)-ptime)*inclength,fits,crtpts)-q_spline(0,fits,crtpts))+q_spline(0,fits,crtpts));
end
